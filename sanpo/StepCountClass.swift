    ...
    func startCatStepCount(catChangeTimeStep: Int, sanpoStepCount: Int) {
        ...
        if (self.nowCatNumber != "") {
            ...
            if (!StepCountClass.isOpenOtherModal) {
                if (Int(CAT_EVENT_STEP) / 3 < self.nowCatStep && !userDefaultUtil.readCatEventFlag(catNumber: self.nowCatNumber, eventNumber: 0)) {
                ...
                } else if ((Int(CAT_EVENT_STEP) / 3) * 2 < self.nowCatStep && !userDefaultUtil.readCatEventFlag(catNumber: self.nowCatNumber, eventNumber: 1)) {
                ...
                } else if (self.nowCatStep > Int(CAT_EVENT_STEP) && !userDefaultUtil.readCatEventFlag(catNumber: self.nowCatNumber, eventNumber: 2)) {
